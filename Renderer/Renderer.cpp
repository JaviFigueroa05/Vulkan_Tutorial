...
void Renderer::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, int32_t mipLevels)
{
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void Renderer::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer &buffer, VkDeviceMemory &bufferMemory)
{
    ...
    VkResult createBufferMemoryResult = vkAllocateMemory(device, &allocInfo, nullptr, &bufferMemory);
    if (createBufferMemoryResult != VK_SUCCESS)
    {
...
void Renderer::createSyncObjects()
{
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
    {
        ...
        VkResult inFlightFenceResult = vkCreateFence(device, &fenceInfo, nullptr, &inFlightFences[i]);
...
int Renderer::rateDeviceSuitability(VkPhysicalDevice device)
{
    ...
    QueueFamilyIndices deviceQueueFamilyIndices = findQueueFamilies(device);
...
void Renderer::createInstance()
{
    ...
    std::vector<VkExtensionProperties> supportenExtensions;
    supportenExtensions = getSupportedVulkanExtensions();

    std::vector<const char *> requiredExtensions;
    requiredExtensions = getRequiredExtensions();

    bool extensionsSupported = validateExtensions(requiredExtensions, supportenExtensions);
    if (!extensionsSupported)
    {
...
bool Renderer::checkValidationLayerSupport()
{
    ...
    std::set<std::string> validationLayersSet(validationLayers.begin(), validationLayers.end());
    for (const auto &layerProperties : availableLayers)
    {
        validationLayersSet.erase(layerProperties.layerName);
    }
    return validationLayersSet.empty();
}
...
std::vector<const char *> Renderer::getRequiredExtensions()
{
    ...
    for (const auto windowExtension : myWindow->GetRequiredExtensions())
    {
...
bool Renderer::validateExtensions(
...
void Renderer::drawFrame()
{
    ...
    VkResult queueSubmmitResult = vkQueueSubmit(graphicsQueue, 1, &submitInfo, inFlightFences[currentFrame]);
    if (queueSubmmitResult != VK_SUCCESS)
    {
